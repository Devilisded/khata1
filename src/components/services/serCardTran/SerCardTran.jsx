...
const SerCardTran = (props) => {
  ...
  return (
    <div className={"cardItem1 cursor-pointer"}>
      {services
        .filter((persons) => persons.serId === serId)
        .map((people) => (
          <div
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col gap-1">
                <span className="text-lg text-slate-700">{people.sname}</span>
              </div>
            </div>
            <button className="flex gap-1 items-center editbtn text-blue-600" onClick={props.edit}>
              <IconEdit />
              Edit Service
            </button>
          </div>
